...
#Args <- c("PDAC","All","Acinar",'CD3D')

# Args <- c("PDAC","All","Acinar,Ductal",'CD3D')
# Args <- c("PDAC","All","Acinar,Ductal,Myeloid,T-cell,B-cell,Plasma,Stellate",'CD3D')
# Args <- c("NSCLC","All","Plasma,Myeloid,Malignant,T-cell",'CD8A')
#Args <- c("CCRCC","All","B-cell,Malignant,T-cell",'CD8A')
...
if (file.exists(genefile)){
  ...
  pic.DEA=function(global.cluster , gene){
    ...
    now.data=p+scale_fill_manual(values = colors)+theme(axis.text.x = element_text(angle=30,hjust=0.5, vjust=0.5,size = rel(1.2),color="black"))
    now.data$labels$title=gene
    maintitle1=paste0(maintitle,'.p1.png')
    ggsave(paste0(resPath,maintitle1),now.data,width = 150,height =110, unit = "mm", dpi=100)
    ...
    ggsave(paste0(resPath,'',maintitle2),p,width = 150,height =110, unit = "mm", dpi=100)
