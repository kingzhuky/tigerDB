...
Args <- commandArgs(T)
#Args <- c("TNMD","GBM_PRJNA482620","None","None","0.5","pdf")
#Args <- c("TNMD","GBM_PRJNA482620","None","None","0.5","pdf")
#Args <- c("TNMD","GBM_PRJNA482620","gene","CD3D","0.5","pdf")
#Args <- c("MEI4","Melanoma_PRJEB23709","None","None","0.5","pdf")
...
if(normalMed == "gene"){
  ...
  plot.data$gene.exp <- as.data.frame(plot.data[,..gene])[,1]
}
...
response.surv.plot <- ggsurvplot(sfit, conf.int = TRUE, pval = TRUE, risk.table = TRUE,
                                 ...
                                 title = paste(paste(mergedatasets, collapse = "_"), gene, sep="-"),
...
if(datatype == "png"){
  ...
  dev.off()
  png(file=paste0("./img/",maintitle2,".png"))
  ...
  print(response.surv.plot)
  dev.off()
  pdf(file=paste0("./img/",maintitle2,".pdf"))
  forestplot(tabletext, 
             cochrane_from_rmeta,
             is.summary=c(rep(FALSE,nrow(forest.plot.data.table)),TRUE),
             xlog=T,title = paste(paste(gene,collapse = "_"),"ForestPlot",sep = "-"),
